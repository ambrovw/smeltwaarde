export const af = {
    // Nav
    silver: 'Silwer',
    gold: 'Goud',
    mintageNav: 'Munt Hoeveelhede',

    // Headings
    silverHeading: 'Silwer munt waarde',
    goldHeading: 'Goue munt waarde',

    // Table headers
    coin: 'Munt',
    era: 'Era',
    purity: 'Fynheid',
    weight: 'Gewig (g)',
    silverGrams: 'Silwer (g)',
    goldGrams: 'Goud (g)',
    each: 'Elk',
    quantity: 'Hoeveelheid',
    valueR: 'Waarde (R)',

    // Premium / adjustment
    premium: 'Premie:',
    premiumTooltipSilver: "Pas die silwerprys aan met 'n persentasie om 'n premie of afslag in te reken.",
    premiumTooltipGold: "Pas die goue prys aan met 'n persentasie om 'n premie of afslag in te reken.",

    // Buttons
    showDetails: 'Fyn Details',
    hideDetailsSilver: 'Versteek Era/Fynheid/Gewig/Silwer',
    hideDetailsGold: 'Versteek Era/Fynheid/Gewig/Goud',
    reset: 'Herstel',
    resetTooltipSilver: 'Herstel silwer blad na verstek state',
    resetTooltipGold: 'Herstel goud blad na verstek state',

    // Totals
    totalSilver: 'Totale silwer:',
    totalGold: 'Totale goud:',
    totalValue: 'Totale waarde:',

    // Loading / errors
    loading: 'Laai tans...',
    priceUnavailable: 'Prysdata nie beskikbaar nie',
    fetchFailedSilver: 'Kon nie silwerprys laai nie',
    fetchFailedGold: 'Kon nie goue prysdata laai nie',

    // MuntHoeveelhede
    mintageTitle: 'Munt Hoeveelhede',
    source: 'Bron:',
    year: 'Jaar',
    zarEra: 'ZAR 1892–1898',
    unionEra: 'Suid Afrikaanse Unie (SAU) 1923–1960',
    rsaEra: 'RSA Eerste Desimale Reeks 1961–1964',

    // Coin group labels (keys from coinData that need translation)
    'Bullion en skroot': 'Bullion en skroot',
    'Skroot': 'Skroot',

    // Banner
    banner: 'Verskoning vir die onlangse stilstand — ons is weer aanlyn en reg om te gaan!',
    bannerEngels: 'Nou ook beskikbaar in Engels!',

    // Footer
    footerCta: 'Wil jy dit vir jou webwerf hê? Kontak my.',
    footerModalText: 'Stuur vir my \'n e-pos en ek sal jou help om dit op te stel:',

    // Helmet
    silverTitle: 'Smeltwaarde | Silwer munt smeltwaarde sakrekenaar',
    silverDescription: 'Bereken die smeltwaarde van ou Suid-Afrikaanse silwer munte — ZAR, Unie, RSA, Brits, en VSA.',
    goldTitle: 'Smeltwaarde | Goue munt smeltwaarde sakrekenaar',
    goldDescription: 'Bereken die smeltwaarde van Krugerrands, ZAR Ponde, en goue skroot.',
    mintageHelmetTitle: 'Smeltwaarde | Munt Hoeveelhede — SA silwer munt oplaagsyfers',
    mintageDescription: 'Suid-Afrikaanse silwer munt oplaagsyfers — ZAR, Unie, en RSA desimale reeks.',
}

export const en = {
    // Nav
    silver: 'Silver',
    gold: 'Gold',
    mintageNav: 'Mintage',

    // Headings
    silverHeading: 'Silver coin value',
    goldHeading: 'Gold coin value',

    // Table headers
    coin: 'Coin',
    era: 'Era',
    purity: 'Purity',
    weight: 'Weight (g)',
    silverGrams: 'Silver (g)',
    goldGrams: 'Gold (g)',
    each: 'Each',
    quantity: 'Quantity',
    valueR: 'Value (R)',

    // Premium / adjustment
    premium: 'Premium:',
    premiumTooltipSilver: 'Adjust the silver price by a percentage to factor in a premium or discount.',
    premiumTooltipGold: 'Adjust the gold price by a percentage to factor in a premium or discount.',

    // Buttons
    showDetails: 'Fine Details',
    hideDetailsSilver: 'Hide Era/Purity/Weight/Silver',
    hideDetailsGold: 'Hide Era/Purity/Weight/Gold',
    reset: 'Reset',
    resetTooltipSilver: 'Reset silver page to defaults',
    resetTooltipGold: 'Reset gold page to defaults',

    // Totals
    totalSilver: 'Total silver:',
    totalGold: 'Total gold:',
    totalValue: 'Total value:',

    // Loading / errors
    loading: 'Loading...',
    priceUnavailable: 'Price data unavailable',
    fetchFailedSilver: 'Failed to fetch silver price',
    fetchFailedGold: 'Failed to fetch gold price',

    // MuntHoeveelhede
    mintageTitle: 'Mintage Figures',
    source: 'Source:',
    year: 'Year',
    zarEra: 'ZAR 1892–1898',
    unionEra: 'South African Union (SAU) 1923–1960',
    rsaEra: 'RSA First Decimal Series 1961–1964',

    // Coin group labels
    'Bullion en skroot': 'Bullion & scrap',
    'Skroot': 'Scrap',

    // Banner
    banner: 'Apologies for the recent downtime — we are back online and ready to go!',

    // Footer
    footerCta: 'Want this for your website? Contact me.',
    footerModalText: 'Send me an email and I\'ll help you set it up:',

    // Helmet
    silverTitle: 'Smeltwaarde | Silver coin melt value calculator',
    silverDescription: 'Calculate the melt value of old South African silver coins — ZAR, Union, RSA, British, and USA.',
    goldTitle: 'Smeltwaarde | Gold coin melt value calculator',
    goldDescription: 'Calculate the melt value of Krugerrands, ZAR Ponds, and gold scrap.',
    mintageHelmetTitle: 'Smeltwaarde | Mintage Figures — SA silver coin mintage data',
    mintageDescription: 'South African silver coin mintage figures — ZAR, Union, and RSA decimal series.',
}
